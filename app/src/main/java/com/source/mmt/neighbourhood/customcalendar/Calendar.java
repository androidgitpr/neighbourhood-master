...
public class Calendar extends LinearLayout implements CalendarFragment.OnItemClickedListener {
    ...
    public void addEvent(LocalDate date, CalenderActivity context) {
        //if (!eventDates.contains(date))
        {
            if(eventDates == null)
            eventDates = new ArrayList<>();
            System.out.println("shiva addevent");
            eventDates.add(date);
           // highlightDate(fragments.get(Months.monthsBetween(startDate, date).getMonths()), "" + date.getDayOfMonth(), mContext);
    ...
    public Calendar(Context context, AttributeSet attrs) {
        ...
        startYear = a.getInteger(R.styleable.CalendarDateElement_startYear, 2018);
        endYear = a.getInteger(R.styleable.CalendarDateElement_endYear, 2020);
        ...
        monthYearBackgroundColor = a.getColor(R.styleable.CalendarDateElement_monthYearBackgroundColor, ContextCompat.getColor(context, R.color.pink));
        ...
        todayBackgroundColor = a.getColor(R.styleable.CalendarDateElement_todayBackgroundColor, ContextCompat.getColor(context, R.color.white));
        ...
        expandedCalendarSelectedColor = a.getColor(R.styleable.CalendarDateElement_expandedCalendarSelectedColor, ContextCompat.getColor(context, R.color.amber));
        ...
        calendarSelectedColor = a.getColor(R.styleable.CalendarDateElement_calendarSelectedColor, ContextCompat.getColor(context, R.color.pink));
        eventDotColor = a.getColor(R.styleable.CalendarDateElement_eventDotColor, ContextCompat.getColor(context, R.color.pink));
        initControl(context);
    }
    ...
    private void initControl(Context context) {
        ...
        System.out.println("shiva event dates");


/*
        for(int i = 0; i < cubs.getEvent().size(); i++) {
            System.out.println("shiva event "+cubs.getEvent().get(i).getDate());
            ...
            if (calendarListener != null) {
                //LocalDate event = new LocalDate(cubs.getEvent().get(i).getDate().)

                //calendarListener.onDateChange(eventDates.get(i));
...
//        highlightDate(fragments.get(Months.monthsBetween(startDate, d).getMonths()), "" + d.getDayOfMonth(), context);
    ...
    private void setUpRecyclerView(final Context context) {
        ...
        //System.out.println("shiva event size "+cubs.getEvent().size());
        //if(cubs.getEvent().size() != 0)
            mRecyclerView.setAdapter(mAdapter);
        mLayoutManager.scrollToPositionWithOffset(Days.daysBetween(startDate, todayDate).getDays() - 1, 0);
        mAdapter.notifyDataSetChanged();
        LocalDate d = dates.get(0);
        highlightDate(fragments.get(Months.monthsBetween(startDate, d).getMonths()), "" + 1, context);
        ...
        mRecyclerView.addOnScrollListener(new RecyclerView.OnScrollListener() {
            ...
            public void onScrolled(RecyclerView recyclerView, int dx, int dy) {
                ...
                System.out.println("shiva addOnScrollListener "+scrollPager);
                if (scrollPager) {
                    int position = mLayoutManager.findFirstVisibleItemPosition() + 1;
                    LocalDate d = dates.get(position);
                    int year = d.getYear();
                    int month = d.getMonthOfYear();
                    System.out.println("shiva addOnScrollListener "+month);
                    setMonthYearText(getMonthName(month), "" + year);
                    mContext.updateEvents(year,month);
                    changeDetected = true;
    ...
    private void highlightDate(CalendarFragment f, String date, Context context) {
        System.out.println("shiva calling highlight"+date);
